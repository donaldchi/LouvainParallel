...
public class Community {
	...
	public def this(filename:String, type_file:int, passTimes:int, precision:double)
	{
		...
		// n2c_p = new HashMap[int, int]();
		// totin_p = new HashMap[int, Tuple2[double,double]]();
		//Console.OUT.println("node, comm, tot, in::");
		...
		{			
			...
			//inside_node(i) = 0.0;
			//Console.OUT.println( i + ", " + n2c(i) + ", " + tot(i) + ", " + inside(i));
			
			// n2c_p.put(i, i);
			// totin_p.put(i, new Tuple2(tot(i), inside(i)));
		}
		nb_pass = passTimes;
		min_modularity = precision;
		m2 = g.total_weight;
	...
	{  //using when reset graph, precision == minm(in c++ version)
		...
		comm_nodeList_Total = new Array[ArrayList[int]]( size+1 );
		//comm_nodeList_Total = commnodeInfo;
		// n2c_p = new HashMap[int, int]();
		// totin_p = new HashMap[int, Tuple2[double,double]]();
		/*Console.OUT.println("weights::");
		for ( i=0; i <g.weights.size(); i++) 
		{
			Console.OUT.println(i + ", " + g.weights(i));
		}
		Console.OUT.println("------------------------------------");*/
		//Console.OUT.println("node, comm, tot, in::");
	...
	@Inline public def neigh_comm( node:int, start_p:int, end_p:int, n2c_p:HashMap[int, int], totin_p:HashMap[int, Tuple2[double, double]], gg:Graph, nodeBand:int )
	{
		...
		//Console.OUT.println("Place, size::" + neigh_weight.size + "," +here.id);

		for ( i=0; i<neigh_last; i++) 
		{
			neigh_weight(neigh_pos(i)) = -1.0;
		}
		//Console.OUT.println("node::" + node);
	...
	public def Init_Info(start_p:int, end_p:int, band:int)  
	{
		...
		//Console.OUT.println("0");
		//Console.OUT.println("Init  node, comm, tot, in::");
		...
		/*Console.OUT.println("-----------");
		for (var i:int=0; i<gg.links.size(); i++) 
		{	
			var weight:double = 0.0;
			if(gg.weights.size()>0)
			  weight = gg.weights(i);
			Console.OUT.println("node, links, weight::" + (i+(placeid-1)*band) + ", " + gg.links(i) + ", " + weight);	
		}
		Console.OUT.println("-----------");*/
	...
	public def one_level_parallel(total_weight:double, nodeSize:int, precision:double, start_p:int, end_p:int, band:int)
	{
    	...
    	for (var nodeNum:int=start_p; nodeNum<end_p;  nodeNum++) 
    	{
    		...
    		/*if (level ==2 )
    			{
    				//Console.OUT.println("node, w_degree::" + nodeNum + ", " + comm_p.w_degree);
    				startIndex = gg.degrees(node) - gg.degrees(0);
            		deg = gg.degrees(node+1) - gg.degrees(0);

            		for (var i:int = startIndex; i<deg; i++) 
            			{
                			Console.OUT.println("nodeNum, i, weights::" + nodeNum + ", " + i + ", " + gg.weights(i));
            			}  
    			}*/
    		...
    		/*if (level==2) {
    			Console.OUT.println("degrees_p");

    			for (var i:int=0; i<degrees_p.size(); i++) 
    			{
    				Console.OUT.println(i + ", " + degrees_p(i));	
    			}
    		...
    		for ( i=0; i< comm_p.neigh_last; i++) 
 			{
	 			...
	 			/*if(level==2 && a<10)
	 			{
	 				Console.OUT.println("totc, dnodecomm, w_degree" + totin_p.get(neigh_comm_tmp).value.val1 + ", " + comm_p.neigh_weight(comm_p.neigh_pos(i)) + ", " + comm_p.w_degree);
	 				a++;
	 			}*/
				...
				/*if(a<15)
						Console.OUT.println("node, neigh, comm, neigh_comm, increase, Place::" + nodeNum + ", " + neigh + ", " + comm_p.node_comm + ", " + neigh_comm_tmp + ", " + comm_p.increase + ", " + here.id);
				a++*/
				/*if (level ==2 ) 
						Console.OUT.println("node, comm, neigh_comm, increase::" + nodeNum + ", " + comm_p.node_comm + ", " + neigh_comm_tmp + ", " + comm_p.increase);*/
    		...
    		{
	    		Console.OUT.println("-------");
	 			Console.OUT.println("node, comm, neigh_comm, best_increase::" + nodeNum + ", " + comm_p.node_comm + ", " + comm_p.best_comm  + ", " + comm_p.best_increase);
	    		Console.OUT.println("-------------------------------------------------");
	    	}*/
	...
	public def one_level( level:int )
	{
		...
		//
		/*for (var i:int=0; i<g.links.size(); i++) {
			var weight:double = 0.0;
			if (g.weights.size()>0) {
				 weight = g.weights(i);
			}
			Console.OUT.println("node, links, weight::" + i + ", " + g.links(i) + ", " + weight);
		}*/
		//
		...
		do{
			...
			/*val moves = MOVES();
			moves(0) = one_level_parallel(m2, min_modularity, 0, size+1, band);*/
