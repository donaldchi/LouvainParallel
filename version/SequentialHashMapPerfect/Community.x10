...
public class Community {
	...
	public def Init_Info(start_p:int, end_p:int, band:int)  
	{
		...
		for (var nodeNum:int=start_p; nodeNum < end_p; nodeNum++) 
		{
			...
			val newNodeNum:int = nodeNum - (placeid)*band;
	...
	/*public def one_level_parallel(total_weight:double, precision:double, start_p:int, end_p:int, band:int)
	{
    	...
    	for (var nodeNum:int=start_p; nodeNum<end_p;  nodeNum++) 
    	{
    		...
    		for (e in comm_p.neigh_weight.entries())
 			{
				...
				Console.OUT.println("node, neigh, comm, neigh_comm, increase::" + nodeNum + ", " + neigh + ", " + comm_p.node_comm + ", " + neigh_comm_tmp + ", " + comm_p.increase);
    		...
    		Console.OUT.println("-------");
 			...
 			Console.OUT.println("node, neigh, comm, neigh_comm, best_increase::" + nodeNum + ", " + neigh + ", " + comm_p.node_comm + ", " + comm_p.best_comm  + ", " + comm_p.best_increase);
    		Console.OUT.println("-------------------------------------------------");
	...
	public def one_level(  )
	{
		...
		do{
	        ...
	        for (var nodeNum:int=0; nodeNum<=size;  nodeNum++) 
	    	{
	  			val nodeBand:int=0;
	    		...
	    		node_comm = n2c_p.get(nodeNum).value;
	    		w_degree = g.weighted_degree(newNodeNum);

	    		/*if (level ==2 )
    			Console.OUT.println("node, w_degree::" + nodeNum + ", " + w_degree);*/
	    		
	    		neigh_comm(nodeNum, 0, size+1, n2c_p, totin_p, g, nodeBand);

	    		remove(newNodeNum, nodeNum, node_comm, neigh_weight(node_comm), totin_p, g, n2c_p);
	    		...
	    		best_increase = 0.0;
	 			...
	 			for ( i=0; i<neigh_last; i++) 
	 			{
		 			...
		 			val neigh_comm_tmp:int = neigh_pos(i);

					 increase =  modularity_gain(newNodeNum, totin_p.get(neigh_comm_tmp).value.val1, neigh_weight(neigh_pos(i)),  w_degree);
					
		 			if (( increase >  best_increase)) 
		 			{
						 best_comm = neigh_comm_tmp;
						 //best_nblinks = e.getValue();
	 			 ...
	 			 insert(newNodeNum, nodeNum,  best_comm,  best_nblinks, totin_p, g, n2c_p);
