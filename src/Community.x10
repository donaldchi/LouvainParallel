...
public class Community {
   ...
   //Use master place, but compute tot, in in each
	public def Init_Info_PE(start_p:int, end_p:int, band:int)  
	{
		//use this function from the second pass
		//init info per place 
		//save segment node`s info
		...
		{
			//val newNodeNum:int = nodeNum - (placeid-1)*band;
			var comm:int = n2c_p.get(nodeNum).value;
			var tot_tmp:double = tot(comm);
			var in_tmp:double =  inside(comm);
			check_p.put(nodeNum, false);
			totin_p.put(comm, new Tuple2(tot_tmp, in_tmp));
			...
			for (var k:int = start_pp; k <end_ppp; k++) 
			{
				...
				if( linkNode < start_p || linkNode >= end_p)
				{
						val neigh_comm:int = n2c_p.get(linkNode).value;
						totin_p.put(neigh_comm, new Tuple2(tot(neigh_comm), inside(neigh_comm)));					
				}			
			}
		}
		...
		do{
			...
			if (nb_pass_done >= 2 ) {
		        PlaceGroup.WORLD.broadcastFlat(()=>{  	
	            	...
	            	val start_p:int = placeid==0 ? 1 : (placeid-1)*band;
	            	val end_p:int = placeid==0 ? 1 : placeid!=(Place.ALL_PLACES-1) ? (here.id)*band : size+1;
	            	Init_Info_PE(start_p, end_p, band);
	        	});
			}
	        ...
	        //compute tot, in in each
	        var totin_master:Array[HashMap[int, Tuple2[double, double]]] = new Array[HashMap[int, Tuple2[double, double]]](Place.ALL_PLACES);
			val refTotin = new GlobalRef[Array[HashMap[int, Tuple2[double, double]]]](totin_master);
	        PlaceGroup.WORLD.broadcastFlat(()=>{
	        val tt:Team = Team.WORLD;   	
        	val placeid:int = here.id;
        	val start_p:int = placeid==0 ? 1 : (placeid-1)*band;
        	val end_p:int = placeid==0 ? 1 : placeid!=(Place.ALL_PLACES-1) ? (here.id)*band : size+1;
        	SetBorderComm(start_p, end_p, band);
        	val totin_p = TOTIN();
        	val totin_transfer:Array[HashMap[int, Tuple2[double, double]]] = new Array[HashMap[int, Tuple2[double, double]]](1);
        	totin_transfer(0) = totin_p;
			tt.gather(placeid, 0, totin_transfer, 0, (here == refTotin.home) ? refTotin.getLocalOrCopy() : null, placeid==0 ? 0 : (placeid-1), 1);
    		});
	        ...
	        for(var i:int = 0; i<totin_master.size; i++)
    		for ( e in totin_master(i).entries()) {
    			comm = e.getKey();
    			tot(comm) += e.getValue().val1;
    			inside(comm) += e.getValue().val2;
    		}

			var startComm:long = -System.nanoTime();
	        
	        /*//compute tot, in in master place 
	...
	public def SetBorderComm(start_p:int, end_p:int, nodeBand:int){
		//Send Border Info to every node to compute tot, in in each.
		val tt:Team = Team.WORLD;

		Console.OUT.println("band::" + nodeBand);

		var data_receive:Array[HashMap[int, int]] = new Array[HashMap[int, int]](Place.ALL_PLACES);
		for (var i:int = 0; i<data_receive.size; i++) {
			data_receive(i) = new HashMap[int, int]();
		}

		var data_transfer:Array[HashMap[int, int]] = new Array[HashMap[int, int]](Place.ALL_PLACES);
		for (var i:int = 0; i<data_transfer.size; i++) {
			data_transfer(i) = new HashMap[int, int]();
		}
    	...
    	for (var node:int = start_p; node < end_p; node++)
	    {
	    	...
	    	val startIndex = gg.degrees(node%nodeBand) - gg.degrees(0);
		    val deg = gg.degrees(node%nodeBand+1) - gg.degrees(0);

		    for ( var i:int = startIndex; i<deg; i++)
		    {
			
				neigh = gg.links(i);
				srcId = neigh / nodeBand +1 ;
				if (srcId >= Place.ALL_PLACES) srcId -= 1;

				if (placeid != srcId && !data_transfer(srcId).containsKey(node)) {
					data_transfer(srcId).put(node, n2c_p.get(node).value);
				}
		    }
		}
		...
		for (var i:int = 0; i<data_transfer.size; i++) {
			...
			tt.gather(here.id, i, data_transfer, i, data_receive, i, 1);			
		}

		neigh = 0;
		var neigh_comm:int = 0; //store border`s node id and comm;
		//update border`s comm
		for (var i:int = 0; i<data_receive.size; i++)
		for ( e in data_receive(i).entries() ) {
			neigh = e.getKey();
			neigh_comm = e.getValue();
			n2c_p.put(neigh, neigh_comm);
		}

		//compute segment tot, in in each
		comm = 0;
		totin_p.clear();
		var totWeights:double = 0.0; 
		var inWeights:double = 0.0;
		for (var node:int = start_p; node < end_p; node++)
	    {
	    	val startIndex = gg.degrees(node%nodeBand) - gg.degrees(0);
		    val deg = gg.degrees(node%nodeBand+1) - gg.degrees(0);

		    comm = n2c_p.get(node).value;
			
			totWeights = 0.0;
			inWeights = 0.0;

			if (totin_p.containsKey(comm)) 
			{
				totWeights= totin_p.get(comm).value.val1;
				inWeights = totin_p.get(comm).value.val2;				
			}
		    ...
		    totin_p.put(comm, new Tuple2(totWeights, inWeights));
		}
...
/*		for (var node:int = start_p; node < end_p; node++)
	    {
			...
			totWeights = totin_p.get(comm).value.val1;
			inWeights = totin_p.get(comm).value.val2;;
		    Console.OUT.println("here.id, comm, tot, in:: " + here.id + ", " + comm + ", " + totWeights + ", " + inWeights);
		}*/
